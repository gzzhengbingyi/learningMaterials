$(document).ready(function() {
    ...
    var myCookie = function(key, value, options) {
    ...
    myCookie('cookie_test','This is test cookie');
    ...
    var onlineUrls = [];
    var filterInterface = function(val){
        if (val === '0') {
            $('.j-mobile, .j-pc').hide();
            $('.j-private').show();
        } else {
    ...
    var initPage = function() {
        ...
        $('#selectMember').on('click', function(event) {
            yxwork.selectMember({
                ...
                selectMyself: $('#cbx-myself')[0].checked,
                success: function(res) {
                    memberIds = [];
        ...
        $('#selectNoticeMember').on('click', function(event) {
            yxwork.selectNoticeMember({
                title: $('#ipt-notice-sel-title').val(),
                btnText: $('#ipt-notice-sel-btnText').val(),
                selectedIds: noticeMemberIds,
                success: function(res) {
                    noticeMemberIds = [];
                    var list = res; //返回选中的成员列表[{openid:'联系人openid',name:'联系人姓名',headImg:'联系人头像url'}]
                    if (list && list.length > 0) {
                        var liHtml = '';
                        for (var i = 0, len = list.length; i < len; i++) {
                            noticeMemberIds.push(list[i].openid);
                            liHtml += '<li><img src="' + list[i].headImg + '" alt=""><p>' + list[i].name + '</p></li>';
                        };
                        $('#noticeSelList').html(liHtml);
                    }
                }
            });
        });
        ...
        $('#showLoading').on('click', function(event) {
            yxwork.showLoading({
                hideIcon: $('#cbx-hideIco')[0].checked,
                message: $('#ipt-toast-loading').val() //提示内容
            });
        });
        ...
        $('#getWifiList').on('click', function(event) {
            var curElem = $(this);
            // var res = [{name:1,bssid:2,signalLevel:23},{name:11,bssid:21,signalLevel:231},];
            yxwork.getWifiList({
                success: function(res) {
                    var htmlStr = '';
                    for (var i = 0, len = res.length; i < len; i++) {
                        htmlStr += '<li>' + res[i].name + ',bssid:' + res[i].bssid + '</li>';
                    };
                    curElem.siblings('ul').html(htmlStr);
                }
            });
        });
        $('#getWifiDetail').on('click', function(event) {
            var curElem = $(this);
            // var res = {name:1,bssid:2,ssid:3,signalLevel:23};
            yxwork.getWifiDetail({
                success: function(res) {
                    var htmlStr = res.name + ',bssid:' + res.bssid;
                    curElem.siblings('p').html(htmlStr);
                }
            });
        });
        $('#getMobileId').on('click', function(event) {
            yxwork.getMobileId({
                success: function(res) {
                    alert(JSON.stringify(res));
                }
        ...
        $('#openViewContainer').on('click', function(event) {
            var _data = {};
            var key = $('#ipt-sender-key').val(),
                value = $('#ipt-sender-value').val();
            if (key && value) {
                _data[key] = value;
            }
            yxwork.openViewContainer({
                data: _data,
                url: 'https://debug.qiye.yixin.im/pages/container.html',
                // url: 'http://10.240.130.14:8989/pages/container.html',
                onBack: function(res) {
                    $('#back-data').show().find('div').text(JSON.stringify(res));
                }
            });
        });
